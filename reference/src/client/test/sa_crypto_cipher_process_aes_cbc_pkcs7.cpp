...
namespace {
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcPkcs7InvalidBytesToProcess) {
        ...
        sa_status status = sa_crypto_cipher_init(cipher.get(), SA_CIPHER_ALGORITHM_AES_CBC_PKCS7, cipher_mode, *key,
                &parameters);
    ...
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcPkcs7FailsInvalidOutLength) {
        sa_buffer_type const buffer_type = std::get<0>(GetParam());
        sa_cipher_mode const cipher_mode = std::get<1>(GetParam());
        auto clear_key = random(SYM_128_KEY_SIZE);
    ...
    TEST_F(SaCryptoCipherSvpOnlyTest, initAesCbcPkcs7FailsDecryptInvalidRightsSvpOptionalNotSet) {
        auto clear_key = random(SYM_128_KEY_SIZE);
        ...
        auto iv = random(AES_BLOCK_SIZE);
        sa_cipher_parameters_aes_cbc parameters = {iv.data(), iv.size()};
        sa_status status = sa_crypto_cipher_init(cipher.get(), SA_CIPHER_ALGORITHM_AES_CBC_PKCS7,
