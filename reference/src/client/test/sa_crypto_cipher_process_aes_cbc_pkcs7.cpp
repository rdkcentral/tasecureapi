...
namespace {
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcPkcs7BadBytesToProcess) {
    ...
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcPkcs7FailsBadOutLength) {
        sa_buffer_type buffer_type = std::get<0>(GetParam());
        sa_cipher_mode cipher_mode = std::get<1>(GetParam());
        auto clear_key = random(SYM_128_KEY_SIZE);
    ...
    TEST_F(SaCryptoCipherSvpOnlyTest, initAesCbcPkcs7FailsDecryptBadRightsSvpOptionalNotSet) {
        auto clear_key = random(SYM_128_KEY_SIZE);
