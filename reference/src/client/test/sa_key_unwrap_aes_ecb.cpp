...
namespace {
    INSTANTIATE_TEST_SUITE_P(
            ...
            SaKeyUnwrapAesEcbTest,
            ::testing::Values(
                    SA_CIPHER_ALGORITHM_AES_ECB,
                    SA_CIPHER_ALGORITHM_AES_ECB_PKCS7));

    TEST_P(SaKeyUnwrapAesEcbTest, failsNullKey) {
    ...
    TEST_P(SaKeyUnwrapAesEcbTest, failsNullRights) {
        auto cipher_algorithm = GetParam();
        std::vector<uint8_t> const clear_key = random(SYM_128_KEY_SIZE);
        std::shared_ptr<sa_key> wrapping_key;
        std::vector<uint8_t> clear_wrapping_key;
    ...
    TEST_P(SaKeyUnwrapAesEcbTest, failsNullIn) {
        auto cipher_algorithm = GetParam();
        std::vector<uint8_t> const clear_key = random(SYM_128_KEY_SIZE);
        std::shared_ptr<sa_key> wrapping_key;
        std::vector<uint8_t> clear_wrapping_key;
    ...
    TEST_P(SaKeyUnwrapAesEcbTest, failsInLength) {
        auto cipher_algorithm = GetParam();
        std::vector<uint8_t> const clear_key = random(SYM_128_KEY_SIZE);
        std::shared_ptr<sa_key> wrapping_key;
        std::vector<uint8_t> clear_wrapping_key;
    ...
    TEST_P(SaKeyUnwrapAesEcbTest, failsWrappingKeyOutsideValidTimeBefore) {
        auto cipher_algorithm = GetParam();
        std::vector<uint8_t> wrapped_key = random(AES_BLOCK_SIZE);
    ...
    TEST_P(SaKeyUnwrapAesEcbTest, failsWrappingKeyOutsideValidTimeAfter) {
        auto cipher_algorithm = GetParam();
        std::vector<uint8_t> wrapped_key = random(AES_BLOCK_SIZE);
    ...
    TEST_P(SaKeyUnwrapAesEcbTest, failsWrappingKeyNotAes) {
    ...
    TEST_P(SaKeyUnwrapAesEcbTest, failAesEcbPkcsPadding) {
        ...
        if (cipher_algorithm == SA_CIPHER_ALGORITHM_AES_ECB_PKCS7) {
            ...
            sa_status status = wrap_key(wrapping_key, clear_wrapping_key, wrapped_key, wrapping_parameters,
                    SYM_128_KEY_SIZE, clear_key, SA_CIPHER_ALGORITHM_AES_ECB, SA_DIGEST_ALGORITHM_SHA1,
