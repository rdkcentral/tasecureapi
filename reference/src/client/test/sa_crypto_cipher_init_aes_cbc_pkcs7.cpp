...
namespace {
    TEST_F(SaCryptoCipherWithoutSvpTest, initAesCbcPkcs7FailsInvalidKeySize48) {
        ...
        sa_status const status = sa_crypto_cipher_init(cipher.get(), SA_CIPHER_ALGORITHM_AES_CBC_PKCS7,
    ...
    TEST_F(SaCryptoCipherWithoutSvpTest, initAesCbcPkcs7FailsNullIv) {
        auto clear_key = random(SYM_128_KEY_SIZE);
