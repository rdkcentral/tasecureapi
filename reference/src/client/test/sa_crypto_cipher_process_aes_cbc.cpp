...
namespace {
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcFailsInvalidBytesToProcess) {
    ...
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcFailsInvalidOutLength) {
        sa_buffer_type const buffer_type = std::get<0>(GetParam());
        sa_cipher_mode const cipher_mode = std::get<1>(GetParam());
        auto clear_key = random(SYM_128_KEY_SIZE);
    ...
    TEST_F(SaCryptoCipherSvpOnlyTest, initAesCbcFailsDecryptInvalidRightsSvpOptionalNotSet) {
        auto clear_key = random(SYM_128_KEY_SIZE);
        ...
        auto iv = random(AES_BLOCK_SIZE);
        sa_cipher_parameters_aes_cbc parameters = {iv.data(), iv.size()};
        sa_status status = sa_crypto_cipher_init(cipher.get(), SA_CIPHER_ALGORITHM_AES_CBC, SA_CIPHER_MODE_DECRYPT,
                *key, &parameters);
        if (status == SA_STATUS_OPERATION_NOT_SUPPORTED)
            GTEST_SKIP() << "Cipher algorithm not supported";
