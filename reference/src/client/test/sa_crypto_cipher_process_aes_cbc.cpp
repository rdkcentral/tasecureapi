...
namespace {
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcFailsBadBytesToProcess) {
    ...
    TEST_P(SaCryptoCipherWithSvpTest, processAesCbcFailsBadOutLength) {
        sa_buffer_type buffer_type = std::get<0>(GetParam());
        sa_cipher_mode cipher_mode = std::get<1>(GetParam());
        auto clear_key = random(SYM_128_KEY_SIZE);
    ...
    TEST_F(SaCryptoCipherSvpOnlyTest, initAesCbcFailsDecryptBadRightsSvpOptionalNotSet) {
        auto clear_key = random(SYM_128_KEY_SIZE);
