 ...
 * @file sa_provider.h
 *
 * sa_rovider implements an OpenSSL 3 Provider that delegates its implementation to SecApi 3. Users can use the OpenSSL
 * API, but SecApi 3 will be used to perform the cryptographic processing.
 *
 * To use an provider, users must call sa_get_provider(). This will create an OpenSSL Library Context and initialize it
 * with a base provider and the SecApi 3 provider.
 *
 ...
 * // Key can also be passed in to the params parameter of EVP_DigestSignInit_ex.
 ...
 *     OSSL_PARAM_construct_end()};
 * // A clear raw HMAC/CMAC key can alternatively be passed into the key parameter--SA_KEY_DELETE will automatically be
 * // set to true.
 ...
 * // A clear raw symmetric key can alternatively be passed into the key parameter--SA_KEY_DELETE will automatically be
 * // set to true.
