...
sa_status sa_key_get_public(
    ...
    sa_key_get_public_s* key_get_public = NULL;
    void* param1 = NULL;
    sa_status status;
    do {
        CREATE_COMMAND(sa_key_get_public_s, key_get_public);
        if (key_get_public == NULL) {
            ERROR("CREATE_COMMAND failed");
            status = SA_STATUS_INTERNAL_ERROR;
            break;
        }

        key_get_public->api_version = API_VERSION;
        ...
        ta_param params[NUM_TA_PARAMS] = {{key_get_public, sizeof(sa_key_get_public_s)},
                                          ...
                                          {NULL, 0}};
        // clang-format on
        status = ta_invoke_command(session, SA_KEY_GET_PUBLIC, param_types, params);
        if (status != SA_STATUS_OK) {
            ERROR("ta_invoke_command failed: %d", status);
            break;
        }

        *out_length = key_get_public->out_length;
        if (out != NULL)
            COPY_OUT_PARAM(out, param1, key_get_public->out_length);
    } while (false);

    RELEASE_COMMAND(key_get_public);
