                               ...
                               pkcs12_secret_t *secret)
{
    ...
    *p += enc_data_len;
    ...
    if ((ret = mbedtls_asn1_get_tag(&param_p, param_end, &len,
        ...
        // Try SHA-256 (matching the MAC algorithm) - mbedTLS 3.x API
        ...
        size_t* name_length)
{
    ...
    // Get filename and password from environment (matching OpenSSL behavior)
    ...
    if (password == NULL)
        password = "password01234567"; // DEFAULT_ROOT_KEYSTORE_PASSWORD
    ...
    bool requested_common_root = (name != NULL && in_name_length > 0 &&
                                 strncmp(name, "commonroot", strlen("commonroot")) == 0);
